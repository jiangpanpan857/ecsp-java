...
import com.ruowei.ecsp.web.rest.dto.ForestDataAllCityDataByProvinceAndYearDTO;
import com.ruowei.ecsp.web.rest.dto.ForestDataAllCityRankingByProvinceAndYearDTO;
import com.ruowei.ecsp.web.rest.dto.ForestDataAllProvinceDataByYearDTO;
import com.ruowei.ecsp.web.rest.dto.ForestDataAllProvinceRankingByYearDTO;
...
public interface ForestDataRepository extends JpaRepository<ForestData, Long>, QuerydslPredicateExecutor<ForestData> {
    ...
    boolean existsByProvinceIdAndCityIdAndYear(String provinceId, String cityId, String year);

    @Cacheable(cacheNames = FORESTDATA_ALL_PROVINCE_STORAGE_BY_YEAR)
    @Query(
        value = "select f.provinceId as id,f.provinceName as name,sum(f.storage) as value from ForestData f where f.year >= ?1 and f.year <= ?2 group by f.provinceId,f.provinceName order by f.provinceId asc"
    )
    ...
    @Query(
        value = "select f.provinceId as id,f.provinceName as name,sum(f.areaIncrement) as value from ForestData f where f.year >= ?1 and f.year <= ?2 group by f.provinceId,f.provinceName order by f.provinceId asc"
    )
    ...
    @Query(
        value = "select f.cityId as id,f.cityName as name,sum(f.storage) as value from ForestData f where f.provinceId = ?1 and f.year >= ?2 and f.year <= ?3 group by f.cityId,f.cityName order by f.cityId asc"
    )
    ...
    @Query(
        value = "select f.cityId as id,f.cityName as name,sum(f.areaIncrement) as value from ForestData f where f.provinceId = ?1 and f.year >= ?2 and f.year <= ?3 group by f.cityId,f.cityName order by f.cityId asc"
    )
    ...
    @Query(
        value = "select new com.ruowei.ecsp.web.rest.dto.ForestDataAllProvinceRankingByYearDTO(0,f.provinceId,f.provinceName,sum(f.storage)) from ForestData f where f.year >= ?1 and f.year <= ?2 group by f.provinceId, f.provinceName order by sum(f.storage) desc"
    )
    ...
    @Query(
        value = "select new com.ruowei.ecsp.web.rest.dto.ForestDataAllProvinceRankingByYearDTO(0,f.provinceId,f.provinceName,sum(f.areaIncrement)) from ForestData f where f.year >= ?1 and f.year <= ?2 group by f.provinceId, f.provinceName order by sum(f.areaIncrement) desc"
    )
    ...
    @Query(
        value = "select new com.ruowei.ecsp.web.rest.dto.ForestDataAllCityRankingByProvinceAndYearDTO(0,f.cityId,f.cityName,sum(f.storage)) from ForestData f where f.provinceId = ?1 and f.year >= ?2 and f.year <= ?3 group by f.cityId, f.cityName order by sum(f.storage) desc"
    )
    ...
    @Query(
        value = "select new com.ruowei.ecsp.web.rest.dto.ForestDataAllCityRankingByProvinceAndYearDTO(0,f.cityId,f.cityName,sum(f.areaIncrement)) from ForestData f where f.provinceId = ?1 and f.year >= ?2 and f.year <= ?3 group by f.cityId, f.cityName order by sum(f.areaIncrement) desc"
    )
    ...
    @Query(value = "select sum(f.storage) from ForestData f where f.provinceId = ?1 and f.year = ?2")
    ...
    @Query(value = "select sum(f.areaIncrement) from ForestData f where f.provinceId = ?1 and f.year = ?2")
    ...
    @Query(value = "select sum(f.storage) from ForestData f where f.cityId = ?1 and f.year = ?2")
    ...
    @Query(value = "select sum(f.areaIncrement) from ForestData f where f.cityId = ?1 and f.year = ?2")
